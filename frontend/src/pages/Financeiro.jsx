  ...
  useEffect(() => {
    async function fetchFinanceiro() {
      ...
      try {
        ...
        const res = await axios.get(`${API_BASE}/api/user/financeiro`, { headers });
        setFinanceiro(res.data.financeiro || []);
      } catch (err) {
        setErro("Erro ao carregar extrato financeiro.");
      }
      setLoading(false);
    }
    fetchFinanceiro();
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="text-left text-gray-600 border-b">
                  ...
                  <th className="py-2">Valor (USD)</th>
              ...
              <tbody>
                {financeiro.length === 0 ? (
                  ...
                  <tr key={idx}>
                    <td>{f.data}</td>
                    <td>{f.descricao}</td>
                    <td className={f.valor > 0 ? "text-green-600" : "text-red-600"}>
                      {f.valor}
                    </td>
                    <td>{f.tipo}</td>
                    <td>{f.status}</td>
