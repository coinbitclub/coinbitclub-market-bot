...
async function getMarketPulse() {
  try {
    ...
    if (Array.isArray(data)) {
      ...
      // Se for um objeto, tentar extrair array de diferentes propriedades
      if (Array.isArray(data.data)) {
        ...
        console.error('⚠️ Market Pulse: Estrutura de dados não reconhecida:', Object.keys(data));
        return { positivePercentage: 50, trend: 'NEUTRAL' };
      }
    } else {
      console.error('⚠️ Market Pulse: Resposta inválida da Binance API:', typeof data);
      return { positivePercentage: 50, trend: 'NEUTRAL' };
    }
    ...
    if (!Array.isArray(tickers) || tickers.length === 0) {
      console.error('⚠️ Market Pulse: Nenhum ticker válido encontrado');
      return { positivePercentage: 50, trend: 'NEUTRAL' };
    }
    
    // Filtrar apenas pares USDT com validação extra
    ...
    if (usdtPairs.length === 0) {
      console.error('⚠️ Market Pulse: Nenhum par USDT válido encontrado');
      return { positivePercentage: 50, trend: 'NEUTRAL' };
    }
    ...
    top100.forEach(ticker => {
      try {
        if (!ticker || !ticker.priceChangePercent || !ticker.quoteVolume) return;
        
        const priceChange = parseFloat(ticker.priceChangePercent);
        const volume = parseFloat(ticker.quoteVolume);
        
        if (isNaN(priceChange) || isNaN(volume)) return;
    ...
    if (validPairs === 0) {
      console.error('⚠️ Market Pulse: Nenhum par válido para análise');
      return { positivePercentage: 50, trend: 'NEUTRAL' };
    }
    
    const positivePercentage = (positive / validPairs) * 100;
    const volumeWeightedDelta = totalVolume > 0 ? ((positiveVolume / totalVolume) - 0.5) * 100 : 0;
    
    let trend = 'NEUTRAL';
    if (positivePercentage > 60) trend = 'BULLISH';
    else if (positivePercentage < 40) trend = 'BEARISH';
    
    console.log(`📊 Market Pulse: ${positive}/${validPairs} positivos (${positivePercentage.toFixed(1)}%)`);
    
    return {
      totalCoins: validPairs,
      positiveCoins: positive,
      negativeCoins: negative,
      positivePercentage,
      volumeWeightedDelta,
      trend,
      source: 'binance_24hr',
...
app.post('/api/webhooks/signal', async (req, res) => {
  try {
    ...
    // Atualizar estatísticas de webhook
    ...
    // ===== PROCESSAMENTO REAL DE TRADING COM TODOS OS SERVIÇOS =====
    let tradingResults = { processed: 0, errors: [], execution_time: 0 };
    const startTime = Date.now();
    
    try {
      console.log('🚀 INICIANDO PROCESSAMENTO DE TRADING ENTERPRISE...');
      tradingResults = await processSignalWithRealTrading(signal);
      tradingResults.execution_time = Date.now() - startTime;
      console.log('✅ TRADING ENTERPRISE PROCESSADO:', tradingResults);
      ...
      await pool.query(`
        INSERT INTO system_monitoring (event_type, details)
        VALUES ('WEBHOOK_PROCESSED', $1)
      `, [JSON.stringify({
        signal_symbol: signal.symbol,
        users_processed: tradingResults.processed,
        errors_count: tradingResults.errors?.length || 0,
        processing_time: new Date().toISOString()
      })]);
      
    } catch (tradingError) {
      console.error('❌ Erro no trading enterprise:', tradingError);
      
      // Detectar tipos de erro para fallback inteligente
      const errorMessage = tradingError.message.toLowerCase();
      const isConnectivityError = errorMessage.includes('403 forbidden') || 
      ...
      tradingResults = {
        ...
        errors: [{ 
          ...
          type: isConnectivityError ? 'connectivity' : 'general',
          fallback_activated: isConnectivityError
        }],
        execution_time: Date.now() - startTime,
        fallback_reason: isConnectivityError ? 'Exchange connectivity issues - CloudFront/Network' : 'General error'
      };
      ...
      await pool.query(`
        INSERT INTO system_monitoring (event_type, details, success)
        VALUES ('WEBHOOK_ERROR', $1, false)
      `, [JSON.stringify({
        error_type: isConnectivityError ? 'connectivity' : 'general',
        error_message: tradingError.message.substring(0, 500),
        signal: signal,
        timestamp: new Date().toISOString(),
        fallback_activated: isConnectivityError
      })]);
    ...
    res.json({
      ...
      processed_signal: {
        ...
        price: signal.price,
        detected_action: tradingResults.detected_action || 'N/A',
        signal_type: tradingResults.signal_type || 'Unknown'
      },
      trading_results: {
        users_processed: tradingResults.processed,
        errors_count: tradingResults.errors?.length || 0,
        errors: tradingResults.errors,
        execution_time_ms: tradingResults.execution_time
      },
      system_health: {
        market_intelligence_active: marketIntelligenceActive,
        services_running: 8, // Número de serviços ativos
        uptime_hours: (process.uptime() / 3600).toFixed(2)
      }
    });

    console.log('✅ Signal processado com sucesso!');
    console.log(`📊 Resultado: ${tradingResults.processed} usuários processados, ${tradingResults.errors?.length || 0} erros`);
