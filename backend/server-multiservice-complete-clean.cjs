// ?? CoinBitClub Market Bot - Servidor Multiservi?o Completo Railway V3
// Configura??o completa para opera??o multiservi?o com GET e POST
// VERS?O FINAL OTIMIZADA: 2025-07-25
...
console.log('?? INICIANDO SERVIDOR MULTISERVI?O COMPLETO...');
console.log('?? CONFIGURA??ES OTIMIZADAS PARA RAILWAY V3');

// Configura??es de vers?o e identifica??o
const SERVER_VERSION = 'v3.0.0-multiservice-hybrid-' + Date.now();
const SERVER_ID = crypto.randomBytes(16).toString('hex');
const START_TIME = Date.now();

console.log(`?? Vers?o: ${SERVER_VERSION}`);
console.log(`?? Server ID: ${SERVER_ID}`);
console.log(`? Inicializado em: ${new Date().toISOString()}`);

// ===== CONFIGURA??ES MULTIUSU?RIO H?BRIDO =====
...
console.log(`?? Sistema Multiusu?rio: ${SISTEMA_MULTIUSUARIO ? 'ATIVO' : 'INATIVO'}`);
console.log(`?? Modo H?brido: ${MODO_HIBRIDO ? 'ATIVO' : 'INATIVO'}`);
...
// Configura??o de seguran?a com Helmet
app.use(helmet({
...
// CORS otimizado para multiservi?o
app.use(cors({
  origin: function (origin, callback) {
    // Permitir todos os origins em produ??o Railway
    if (!origin || 
...
app.use('/api', createRateLimit(60000, 1000, 'Muitas requisi??es API por minuto'));
...
// Log da configura??o do banco
console.log('??? Configura??o PostgreSQL Multiservi?o:');
...
// Fun??o de teste de conex?o robusta
async function testDatabaseConnection() {
  try {
    const client = await pool.connect();
    const result = await client.query('SELECT NOW() as current_time, version() as pg_version');
    console.log('? PostgreSQL Multiservi?o conectado:', result.rows[0].current_time);
    ...
    console.error('? Erro conex?o PostgreSQL:', error.message);
    return {
      status: 'disconnected',
      error: error.message
    };
...
// Root endpoint - Informa??es completas do servi?o
app.get('/', (req, res) => {
  console.log('?? Root endpoint multiservi?o acessado');
  
  const uptime = Math.floor((Date.now() - START_TIME) / 1000);
  
  res.json({
    service: 'CoinBitClub Market Bot Multiservi?o',
    status: 'active',
...
app.get('/health', async (req, res) => {
  console.log('?? Health check multiservi?o requisitado');
  
  try {
    // Tentar conex?o DB mas n?o falhar se n?o conseguir
    let dbStatus;
    try {
      dbStatus = await testDatabaseConnection();
    } catch (dbError) {
      console.log('?? DB n?o dispon?vel no health check, mas servidor OK');
    ...
    console.log('? Health check multiservi?o OK');
    res.json(health);
    
  } catch (error) {
    console.error('? Health check multiservi?o error:', error);
    res.status(500).json({
      status: 'error',
      service: 'coinbitclub-multiservice',
      version: SERVER_VERSION,
...
app.get('/api/health', async (req, res) => {
  console.log('?? API Health multiservi?o requisitado');
  ...
  try {
    ...
    console.error('? API Health multiservi?o error:', error);
    res.status(500).json({
      status: 'error',
      service: 'coinbitclub-api-multiservice',
      message: error.message,
...
app.get('/api/status', (req, res) => {
  console.log('?? Status multiservi?o endpoint acessado');
...
app.get('/api/metrics', (req, res) => {
  console.log('?? M?tricas multiservi?o requisitadas');
...
// Endpoint gen?rico para recebimento de dados via GET
...
// Endpoint gen?rico para recebimento de dados via POST
...
app.post('/api/webhooks/tradingview', async (req, res) => {
  console.log('?? WEBHOOK TRADINGVIEW MULTISERVI?O RECEBIDO');
...
// Webhook para sinais TradingView - endpoint espec?fico /api/webhooks/signal
...
app.post('/webhook/:signal', async (req, res) => {
  ...
  console.log(`?? WEBHOOK GEN?RICO RECEBIDO: ${signal}`);
  console.log('?? Payload:', JSON.stringify(req.body, null, 2));
  ...
  try {
    ...
    console.log(`? Webhook gen?rico ${signal} processado`);
...
// ===== ENDPOINTS SISTEMA MULTIUSU?RIO H?BRIDO =====

// Status do sistema multiusu?rio
app.get('/api/multiuser/status', async (req, res) => {
  console.log('?? Status sistema multiusu?rio solicitado');
  
  try {
    const client = await pool.connect();
    
    // Verificar usu?rios ativos (consulta simplificada)
    const usersResult = await client.query(`
      SELECT COUNT(*) as total_users
      FROM users
    `);
    
    // Verificar opera??es (consulta simplificada)
    ...
    console.error('? Erro ao obter status multiusu?rio:', error.message);
    res.status(500).json({
      status: 'error',
...
// Listar usu?rios ativos em tempo real
app.get('/api/multiuser/users/active', async (req, res) => {
  console.log('?? Lista de usu?rios ativos solicitada');
  ...
  try {
    ...
    console.error('? Erro ao listar usu?rios ativos:', error.message);
    res.status(500).json({
      status: 'error',
      message: 'Erro ao listar usu?rios ativos',
      error: error.message,
      timestamp: new Date().toISOString()
    });
  }
...
// Opera??es em tempo real por usu?rio
app.get('/api/multiuser/operations/realtime', async (req, res) => {
  console.log('? Opera??es em tempo real solicitadas');
  ...
  try {
    ...
    console.error('? Erro ao obter opera??es em tempo real:', error.message);
    res.status(500).json({
      status: 'error',
      message: 'Erro ao obter opera??es em tempo real',
      error: error.message,
      timestamp: new Date().toISOString()
    });
  }
...
// Configurar chaves API de usu?rio
app.post('/api/multiuser/user/:userId/api-keys', async (req, res) => {
  ...
  console.log(`?? Configurando chaves API para usu?rio ${userId} - Exchange: ${exchange_name}`);
  ...
  try {
    ...
    // Criptografar chaves (simula??o - usar crypto real em produ??o)
...
// Habilitar modo h?brido para usu?rio
app.post('/api/multiuser/user/:userId/hybrid-mode', async (req, res) => {
  const userId = req.params.userId;
  const { enabled = true } = req.body;
  
  console.log(`?? ${enabled ? 'Habilitando' : 'Desabilitando'} modo h?brido para usu?rio ${userId}`);
  
  try {
    const client = await pool.connect();
    
    // Atualizar configura??es do usu?rio
    await client.query(`
    ...
    res.json({
      status: 'success',
      message: `Modo h?brido ${enabled ? 'habilitado' : 'desabilitado'}`,
      user_id: userId,
      hybrid_mode: enabled,
      timestamp: new Date().toISOString()
    });
    ...
    console.error('? Erro ao configurar modo h?brido:', error.message);
    res.status(500).json({
      status: 'error',
      message: 'Erro ao configurar modo h?brido',
      error: error.message,
      timestamp: new Date().toISOString()
    });
  }
...
// Endpoint para processar sinais multiusu?rio
app.post('/api/multiuser/signal/process', async (req, res) => {
  console.log('?? Processando sinal para sistema multiusu?rio');
  ...
  try {
    ...
    // Processar sinal para cada usu?rio
    const results = [];
    for (const userId of users || []) {
      try {
        // Verificar configura??es do usu?rio
        ...
        if (userConfig.rows.length > 0) {
          // Simular processamento da opera??o
          const operation = {
            user_id: userId,
            symbol: symbol,
            action: action,
        ...
        console.error(`? Erro ao processar usu?rio ${userId}:`, userError.message);
        results.push({
          user_id: userId,
          status: 'error',
    ...
    res.json({
      status: 'success',
      message: 'Sinal processado para usu?rios',
    ...
    console.error('? Erro ao processar sinal multiusu?rio:', error.message);
    res.status(500).json({
      status: 'error',
...
// Fun??o para salvar dados no banco
...
// Fun??o para incrementar contadores
...
app.use((error, req, res, next) => {
  console.error('?? Erro na aplica??o multiservi?o:', error);
  incrementCounter(req.app, 'errorCount');
  
  res.status(500).json({
    error: 'Erro interno do servidor multiservi?o',
    message: error.message,
    version: SERVER_VERSION,
    timestamp: new Date().toISOString(),
    server_id: SERVER_ID
...
console.log('?? Configura??o do servidor multiservi?o:');
...
const server = app.listen(PORT, HOST, async () => {
  console.log('?? ===== SERVIDOR MULTISERVI?O RAILWAY INICIADO =====');
  ...
  console.log('?? Endpoints multiservi?o dispon?veis:');
  console.log('');
  console.log('   ?? INFORMA??ES:');
  console.log('   ?? GET  /                              - Status principal');
  console.log('   ?? GET  /health                        - Health check');
  console.log('   ?? GET  /api/health                    - API Health check');
  console.log('   ?? GET  /api/status                    - Status detalhado');
  console.log('   ?? GET  /api/metrics                   - M?tricas do sistema');
  console.log('');
  console.log('   ?? WEBHOOKS:');
  console.log('   ?? POST /api/webhooks/tradingview      - Webhook TradingView');
  console.log('   ?? POST /webhook/:signal               - Webhook gen?rico');
  ...
  console.log('??? Seguran?a: Helmet + Rate Limiting');
  console.log('?? CORS: Otimizado para multiservi?o');
  console.log('? Sistema multiservi?o otimizado!');
  console.log('?? PRONTO PARA RECEBER GET E POST!');
});

// Error handling do servidor
server.on('error', (err) => {
  console.error('? Erro no servidor multiservi?o:', err);
...
// Graceful shutdown
process.on('SIGTERM', () => {
  console.log('?? SIGTERM recebido, encerrando servidor multiservi?o...');
  server.close(() => {
    console.log('? Servidor multiservi?o encerrado graciosamente');
    pool.end(() => {
      console.log('? Pool de conex?es encerrado');
      process.exit(0);
    });
  });
});
...
process.on('SIGINT', () => {
  console.log('?? SIGINT recebido, encerrando servidor multiservi?o...');
  server.close(() => {
    console.log('? Servidor multiservi?o encerrado');
    pool.end(() => {
      console.log('? Pool de conex?es encerrado');
      process.exit(0);
    });
