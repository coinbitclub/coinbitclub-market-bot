# 🔥 S# 2. Remover TODOS os arquivos conflita# 3. Limpar pastas antigas conflitantes
...
}ite-Host "🗑️ Removendo arquivos antigos..." -ForegroundColor Yellow
...
$filesToRemove = @(
    ...
    "*.old",
    "*-backup.*"
)PEZA COMPLETA E FORCE PUSH - SISTEMA V3
...
# 3. Criar package.json LIMPO e SIMPLES
...
$cleanPackageJson | Out-File -FilePath "package.json" -Encoding UTF8
...
Write-Host "`n🎯 PREPARANDO COMMIT FORÇADO..." -ForegroundColor Cyan
...
Write-Host "`n🚀 COMANDOS PARA EXECUTAR:" -ForegroundColor Magenta
Write-Host "1. git add ." -ForegroundColor White
Write-Host "2. git commit -m `"FORCE: Deploy Sistema V3 - Remove arquivos antigos completamente`"" -ForegroundColor White  
Write-Host "3. git push origin main --force" -ForegroundColor White
Write-Host "`n⚠️ IMPORTANTE: Vai executar automaticamente em 3 segundos..." -ForegroundColor Yellow
...
try {
    ...
    git commit -m "FORCE: Deploy Sistema V3 - Remove arquivos antigos completamente" 2>&1 | Out-Host
