...
(async () => {
  try {
    ...
    const modelBuffer = await trainAndSerialize({
      signals,
      metrics,
      fgData,
      operations,
    ...
    const outPath = path.resolve(process.cwd(), 'model.bin');
    fs.writeFileSync(outPath, modelBuffer);
    logger.info(`Model saved to ${outPath}`);
    process.exit(0);
  } catch (e) {
    logger.error(e);
    process.exit(1);
