...
async function criarUsuariosFinal() {
  try {
    console.log('👥 CRIANDO NOVOS USUÁRIOS NO SISTEMA - VERSÃO FINAL');
    ...
    // Adicionar balanço - apenas available_balance
    await pool.query(`
      INSERT INTO user_balances (
        user_id, exchange, currency, available_balance, created_at
      ) VALUES (
        $1, 'bybit', 'BRL', 500.00, NOW()
      )
    `, [palomaId]);
    console.log(`   💰 Balanço de R$500 configurado`);
    
    // Chaves API placeholders
    await pool.query(`
    ...
    await pool.query(`
      INSERT INTO user_balances (
        ...
        $1, 'bybit', 'BRL', 5000.00, NOW()
      )
    `, [ericaId]);
    console.log(`   💰 Balanço de R$5.000 configurado`);
    
    // Chaves API reais da Erica
    await pool.query(`
      INSERT INTO user_api_keys (
        user_id, exchange, api_key, secret_key, 
        is_active, validation_status, environment, created_at, updated_at
    ...
    console.log(`     📧 Email: pamaral15@hotmail.com`);
    console.log(`     🔐 Senha: Diogo1520`);
    console.log('');
    console.log(`   💎 Erica dos Santos Andrade (ID: ${ericaId}) - VIP com R$5.000 + API Keys`);
    console.log(`     📧 Email: erica.andrade.santos@hotmail.com`);
    console.log(`     🔐 Senha: Apelido22@`);
    console.log(`     🔑 API Key: axgdZkzOx55Ay3aqeg`);
    console.log('');
