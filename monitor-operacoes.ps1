# Monitor de Operacoes em Tempo Real - CoinBitClub Market Bot
...
# Funcao para verificar status de servico
function Test-ServiceStatus {
    ...
    try {
        ...
        if ($errorMsg -like "*502*") {
            return @{ Status = "502 BAD GATEWAY"; StatusCode = 502; Color = "Red"; Icon = "[ERROR]" }
        }
        elseif ($errorMsg -like "*timeout*") {
            return @{ Status = "TIMEOUT"; StatusCode = "TIMEOUT"; Color = "Yellow"; Icon = "[TIMEOUT]" }
        }
        elseif ($errorMsg -like "*refused*") {
            return @{ Status = "OFFLINE"; StatusCode = "REFUSED"; Color = "Red"; Icon = "[OFFLINE]" }
        }
        else {
            return @{ Status = "ERROR"; StatusCode = "ERROR"; Color = "Red"; Icon = "[ERROR]" }
        }
    }
}

# Funcao para verificar processos Node.js
    ...
    foreach ($port in $ports) {
        try {
            ...
            $results += @{
                ...
                Icon = if ($connection) { "[OPEN]" } else { "[CLOSED]" }
            }
        }
        catch {
            $results += @{ Port = $port; Status = "ERROR"; Color = "Red"; Icon = "[ERROR]" }
        }
    }
...
# Funcao para obter metricas do sistema
...
# Funcao para verificar logs recentes
function Get-RecentActivity {
    $activities = @()
    
    # Verificar logs reais do sistema
...
" -ForegroundColor Cyan
    ...
    # ===== STATUS DAS PORTAS =====
    Write-Host "`nSTATUS DAS PORTAS" -ForegroundColor Yellow
    Write-Host "=================" -ForegroundColor Yellow
    ...
    # ===== PROCESSOS NODE.JS =====
    Write-Host "`nPROCESSOS NODE.JS ATIVOS" -ForegroundColor Yellow
    Write-Host "========================" -ForegroundColor Yellow
    ...
    if ($nodeProcesses.Count -gt 0) {
        foreach ($process in $nodeProcesses) {
            ...
            Write-Host "[ATIVO] PID: $($process.Id.ToString().PadRight(6)) | Memoria: $($memoryMB.ToString().PadLeft(6)) MB | Runtime: $runtime" -ForegroundColor Green
        }
    } else {
    ...
    # ===== METRICAS DO SISTEMA =====
    Write-Host "`nMETRICAS DO SISTEMA" -ForegroundColor Yellow
    Write-Host "===================" -ForegroundColor Yellow
    
    $metrics = Get-SystemMetrics
    Write-Host "CPU: $($metrics.CPU)%" -ForegroundColor $(if ($metrics.CPU -lt 70) { "Green" } elseif ($metrics.CPU -lt 90) { "Yellow" } else { "Red" })
    ...
    # ===== ATIVIDADE RECENTE =====
    Write-Host "`nATIVIDADE RECENTE" -ForegroundColor Yellow
    Write-Host "=================" -ForegroundColor Yellow
    ...
    # ===== CONTROLES =====
    Write-Host "`nCONTROLES DISPONIVEIS" -ForegroundColor Yellow
    Write-Host "=====================" -ForegroundColor Yellow
    ...
    # ===== URLs DE ACESSO =====
    Write-Host "`nACESSO AOS SERVICOS" -ForegroundColor Yellow  
    Write-Host "===================" -ForegroundColor Yellow
    ...
    Write-Host "`nProxima atualizacao em $RefreshInterval segundos..." -ForegroundColor Gray
    
    # Aguardar antes da proxima atualizacao
