...
exports.runAuditAndPurge = runAuditAndPurge;
var _databaseService = require("../services/databaseService.js");
// Remove dados mais antigos que 72h
async function purgeOldData() {
  await (0, _databaseService.query)(`DELETE FROM signals WHERE time < NOW() - INTERVAL '72 HOURS'`);
  await (0, _databaseService.query)(`DELETE FROM dominance WHERE created_at < NOW() - INTERVAL '72 HOURS'`);
  await (0, _databaseService.query)(`DELETE FROM fear_greed WHERE created_at < NOW() - INTERVAL '72 HOURS'`);
  await (0, _databaseService.query)(`DELETE FROM market WHERE created_at < NOW() - INTERVAL '72 HOURS'`);
}

// Salva snapshot di�rio nas tabelas *_daily (campos alinhados!)
async function snapshotDailyData() {
  await (0, _databaseService.query)(`
