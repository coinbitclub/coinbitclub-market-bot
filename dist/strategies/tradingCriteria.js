...
exports.evaluateExitCriteria = evaluateExitCriteria;
 ...
 */
async function evaluateEntryCriteria(signal) {
  const fg = (await (0, _fearGreedService.getLatestFearGreed)()).value;
  const dom = await (0, _dominanceService.getLatestDominance)();
 ...
 */
function evaluateExitCriteria(position, market) {
  const {
    diff_btc_ema7,
    cruzou_acima_ema9,
    cruzou_abaixo_ema9,
 ...
 */
function calculateTpsl(leverage = 5) {
