...
async function requestV5({
  ...
  try {
    ...
    if (method === 'GET') {
      ...
      response = await _axios.default.post(url, data, {
        params,
        headers
      });
    } else {
...
async function placeMarketOrder({
  ...
  if (process.env.NODE_ENV === 'test') {
    console.log('Modo TESTE: nenhuma ordem real será enviada para Bybit.', {
      symbol,
      side,
      qty,
      price,
  ...
  try {
    ...
    const resp = await _axios.default.post(`${BASE_URL}/v2/private/order/create`, null, {
      params: {
        ...params,
        sign
      }
