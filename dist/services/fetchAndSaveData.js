...
/**
 * Salva o payload de sinal na tabela `signals`
 */
async function saveSignal(body) {
  ...
  await (0, _databaseService.query)(`INSERT INTO public.signals (
       signal_json,
       received_at,
       ticker,
       time,
...
async function fetchAndSaveDominance() {
  ...
  await (0, _databaseService.query)(`INSERT INTO public.dominance (
       timestamp,
       btc_dom,
       eth_dom
     )
...
/**
 * Busca e salva índice Fear & Greed
 */
async function fetchAndSaveFearGreed() {
  ...
  await (0, _databaseService.query)(`INSERT INTO public.fear_greed (
       index_value,
       value_classification,
       timestamp
     )
...
async function fetchAndSaveMarket() {
  ...
  for (const {
    ...
    await (0, _databaseService.query)(`INSERT INTO public.market (
         symbol,
         price,
         timestamp
       )
