...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Apenas verificar token se estiver configurado E se o header estiver presente
    ...
    console.log('🔥 TradingView webhook recebido:', JSON.stringify(signalData, null, 2));
    ...
    // Processar o sinal
    console.log('🚀 Iniciando processamento do sinal...');
    const processedSignal = await processTradingViewSignal(signalData);
    console.log('✅ Sinal processado com sucesso:', processedSignal);
...
async function processTradingViewSignal(signalData: TradingViewSignal) {
  console.log('📊 Processando sinal do TradingView:', signalData.symbol, signalData.action);
  
  try {
    // 1. Salvar sinal no banco de dados
    console.log('💾 Salvando sinal no banco de dados...');
    ...
    console.log('✅ Sinal salvo com ID:', signalId);

    // 2. Processar para Decision Engine
    console.log('🤖 Enviando para Decision Engine...');
    await processSignalForDecisionEngine(signalId, signalData);

    // 3. Acionar processamento automático da fila
    ...
    try {
      ...
      if (response.ok) {
        console.log('✅ Processamento automático acionado com sucesso');
      } else {
        console.log('⚠️ Processamento automático falhou:', response.status);
      }
    } catch (error) {
      console.log('ℹ️ Processamento automático não acionado (background):', error.message);
    }
...
async function processSignalForDecisionEngine(signalId: string, signalData: TradingViewSignal) {
  console.log('🧠 Preparando dados para Decision Engine, signal ID:', signalId);
  ...
  try {
    ...
    console.log('📝 Salvando na fila de processamento...');
    ...
    console.log(`✅ Sinal ${signalId} adicionado à fila do Decision Engine com confiança ${getConfidenceFromAction(signalData.action)}%`);
