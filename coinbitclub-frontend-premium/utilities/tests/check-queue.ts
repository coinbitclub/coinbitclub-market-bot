...
async function checkQueue() {
  try {
    ...
    if (pending.rows.length > 0) {
      ...
      // Processar este sinal pendente
      console.log('\n🚀 Processando sinal pendente...');
      const response = await fetch('http://localhost:3001/api/services/decision-engine', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ queueId: signal.id })
      });
      
      if (response.ok) {
        const result = await response.json();
        console.log('✅ Sinal processado:', JSON.stringify(result, null, 2));
      } else {
        console.log('❌ Erro ao processar:', response.status, await response.text());
      }
    }
    ...
    console.log(`\n📈 Últimos 5 sinais:`);
    recent.rows.forEach((signal: any, i: number) => {
