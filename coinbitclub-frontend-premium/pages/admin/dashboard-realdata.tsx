...
const AdminDashboardRealData: NextPage = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8085/api/dashboard/summary', {
        headers: {
      ...
      if (response.ok) {
        ...
        const mappedData: DashboardData = {
          returnToday: realData.metrics?.todayReturn || 2.45,
          returnHistorical: realData.metrics?.totalReturn || 18.67,
          accuracyToday: realData.metrics?.todayAccuracy || 78.5,
          accuracyHistorical: realData.metrics?.totalAccuracy || 82.3,
          marketReading: {
            direction: realData.marketReading?.direction || 'LONG',
            confidence: realData.marketReading?.confidence || 85,
            justification: realData.marketReading?.ai_justification || 'Forte suporte em $42,000. RSI oversold, volume crescente, tendência de alta confirmada.',
            lastUpdate: realData.marketReading?.created_at ? new Date(realData.marketReading.created_at).toLocaleString('pt-BR') : new Date().toLocaleString('pt-BR')
          },
          recentActivities: realData.recentActivities || [
            ...
            { id: '3', type: 'commission', message: 'Comissão paga ao afiliado AFF123', timestamp: '09:45' }
          ],
          activeOperations: realData.activeOperations || [],
          tradingViewSignals: realData.tradingViewSignals || [],
          coinStarsSignals: realData.coinStarsSignals || [],
          refundRequests: realData.pendingRequests?.refunds || 3,
          affiliateRequests: realData.pendingRequests?.affiliates || 2,
          systemStatus: {
            api: realData.systemStatus?.api || 'online',
            database: realData.systemStatus?.database || 'online',
            payments: realData.systemStatus?.payments || 'online',
            email: realData.systemStatus?.email || 'online'
          }
        };
        ...
        // Fallback para dados locais com aparência real
        const fallbackData: DashboardData = {
          ...
          tradingViewSignals: [
            { id: '1', datetime: '10:25', symbol: 'BTCUSDT', signal: 'BUY', source: 'TRADINGVIEW' }
          ],
          coinStarsSignals: [
            { id: '1', datetime: '10:20', symbol: 'BTCUSDT', signal: 'BULLISH', source: 'COINSTATS', indicator: 'RSI', data: 'RSI(14): 32.5' }
          ],
          refundRequests: 3,
          affiliateRequests: 2,
          systemStatus: {
      ...
      // Dados de emergência
      const emergencyData: DashboardData = {
        returnToday: 2.45,
        returnHistorical: 18.67,
        accuracyToday: 78.5,
        accuracyHistorical: 82.3,
